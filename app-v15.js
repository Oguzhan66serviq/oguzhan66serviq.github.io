...
function replyHtml(quote){
  ...
  return `<p>Guten Tag ${html(name)},</p><p>${html(quote.introduction)}</p><table style="border-collapse:collapse;width:100%"><tbody>${rows}<tr><td colspan="2" style="padding:8px;text-align:right"><b>Gesamt inkl. MwSt.</b></td><td style="padding:8px;text-align:right"><b>${html(euro(quote.gross))}</b></td></tr></tbody></table><p>Das vollständige freibleibende Angebot finden Sie als PDF im Anhang.</p><p>${html(quote.closing)}</p><p><small>Das Angebot steht unter dem Vorbehalt der Verfügbarkeit und abschließenden Bestätigung.</small></p><p>Freundliche Grüße<br><b>${html(state.company.name||state.membership.organizations?.name||'Ihr Catering-Team')}</b></p>`;
}
